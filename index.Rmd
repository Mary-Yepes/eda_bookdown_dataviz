...
El fraude en transacciones con tarjeta de crédito es uno de los principales peligros para las entidades financieras a nivel mundial, por lo que poder detectarlo a tiempo es importante para reducir las pérdidas económicas, proteger a los clientes y mantener la confianza en los sistemas de pago electrónico. El fraude con tarjeta de crédito se refiere al uso ilegal y no autorizado de la tarjeta de crédito o de los datos de la tarjeta de otra persona para realizar transacciones o compras fraudulentas (Microblink, s.f.).

El fraude siempre ha estado presente en la sociedad y ha tomado diversas formas a lo largo del tiempo. El fraude con tarjeta de crédito apareció por primera vez en la década de los 80, cuando se produjo un aumento significativo en el uso de tarjetas de crédito, débito y prepago a nivel mundial (Buonaguidi, 2017). Debido a este crecimiento, las instituciones financieras comenzaron a desarrollar sistemas para detectar actividades sospechosas. Inicialmente, estos sistemas se basaban en reglas predefinidas y revisiones manuales basadas en el conocimiento de expertos para identificar comportamientos inusuales en las transacciones. Posteriormente, se empezaron a utilizar métodos estadísticos tradicionales de clasificación, como el análisis discriminante lineal y la regresión logística, los cuales demostraron ser efectivos en muchas aplicaciones. Con el tiempo, se incorporaron técnicas más avanzadas como redes neuronales y otros métodos de aprendizaje automático, así como enfoques basados en reglas (rule-based methods) como Bayes o RIPPER, que permitieron mejorar la detección de patrones asociados al fraude (Bolton & Hand, 2002; Tarazona Nieto et al., 2022).
...
plots <- lapply(seq_along(cols), function(i) {
  ggplot(data.frame(x = as.factor(df$Class), y = pc_df[[cols[i]]]), aes(x = x, y = y)) +
    ...
    scale_fill_manual(
      values = c("0" = "#1f77b4", "1" = "#d62728")  # azul = no fraude, rojo = fraude
    ) +
    labs(title = cols[i], x = NULL, y = NULL) +
    theme_bw() +
    theme(
...
# Seleccionar solo V1–V28
pc_df <- df %>% select(starts_with("V"))

plots <- lapply(colnames(pc_df), function(col) {
  
  ggplot(df, aes(x = Class, y = .data[[col]])) +
    geom_boxplot(fill = "steelblue", outlier.size = 0.3) +
    labs(title = col, y = NULL, x = NULL) +
    scale_x_discrete(labels = c("0" = "No fraude",
                                "1" = "Fraude")) +
    theme_minimal() +
...
# Dividir en páginas de 8 plots
n <- 8

for(i in seq(1, length(plots), by = n)) {
  print(wrap_plots(plots[i:min(i+n-1, length(plots))], ncol = 4))
}
```
...
# quitar Class
df_no_fraude_corr <- df_no_fraude[, !(names(df_no_fraude) %in% "Class")]
df_fraude_corr    <- df_fraude[, !(names(df_fraude) %in% "Class")]

# correlaciones Spearman
corr_no_fraude <- cor(df_no_fraude_corr,
                      method = "spearman",
                      use = "complete.obs")

corr_fraude <- cor(df_fraude_corr,
                   method = "spearman",
                   use = "complete.obs")

# Función para graficar heatmap triangular
...
# Crear gráficos
g1 <- plot_heatmap(corr_no_fraude,
                   "Correlación Spearman - No Fraude")

g2 <- plot_heatmap(corr_fraude,
                   "Correlación Spearman - Fraude")
...
layout(
  ...
  xaxis = list(title = "Hora del día (0–23)"),
